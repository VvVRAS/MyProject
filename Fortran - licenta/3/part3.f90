...
do i=1,80
	v(i)=v2+(pi*d**2)/4*R*(1+lamda/4-cos(thetha(i)*rad)-lamda/4*cos(2*thetha(i)*rad))
	p(i)=(mamp*Ram*temp(i))/v(i)
	!print*, "	----------------------------------------------------------"
	!print*, i, "| p(i) |", p(i), "| v(i) |", v(i)
	write(81,*) p(i), v(i)
	print*, "#####", i
end do
!print*, "	----------------------------------------------------------"
...
!#############################Matricea-1#############################
	...
	!print*, "Afisarea sistemului"
	!print*, a111, a112, a113, b11
	!print*, a121, a122, a123, b12
	!print*, a131, a132, a133, b13
...
! print solutions
	...
	close (122)
	print*, " "
	print*, "SOLUTIA ESTE:-20216136.0; 14037573.0; 75149.2969"
	print*, " "
...
!do i=1,2
!if (i==1) then
!	pres_adm(1)=ae1*V1**2+be1*V1+ce1
!	else
!	pres_adm(2)=ae1*V2**2+be1*V2+ce1
!endif
!enddo
!print*, "pres_adm(V1)=", pres_adm(1)
!print*, "pres_adm(V2)=", pres_adm(2)
...
OPEN(211,FILE='input2.txt')
	...
	!print*, "Afisarea sistemului"
	!print*, a211, a212, a213, b21
	!print*, a221, a222, a223, b22
	!print*, a231, a232, a233, b23
...
OPEN(231,FILE='input4.txt')
	...
	!print*, "Afisarea sistemului"
	!print*, a411, a412, a413, b41
	!print*, a421, a422, a423, b42
	!print*, a431, a432, a433, b43
...
!####################################################################
!!!! GRAFIC CICLUL REAL INCHIS
!####################################################################

!valorile lui p_a/e, l_lev_adm/ev trebuie bagate intr un fisier txt si apoi
!bagate intr o alta variabila si cu acea variabila sa fac graficul
! oare???!!! I'm the WRAS War of Real All Sec
sum_p_a=0
do i=1,180
	teta_adm(i) = i
	teta=i
...
!p_m_ciclu_secundar=((sum(p_a(i))/181)+(sum(p_e(i))/181)+(sum(p_la(i))/71)+(sum(p_le(i))/41))/4
...
!sa l bag intr un fisier si dupa ... ce face cu el?
!T(teta)

print*, " m(teta)	  ", "  f_ip(teta)	  ", "   Tg(teta)", "		z(teta)", "		K(teta)	", "	norm(teta)	", "f_sigma(teta)	"
!do teta=250,500
do i=1,79
if (250<=thetha(i).and.thetha(i)<360) then
	t_thetha(i)=(28*thetha(i)-1500)/11
	else if (360<=thetha(i).and.thetha(i)<400) then
		t_thetha(i)=38*thetha(i)-12900
		else if (400<=thetha(i).and.thetha(i)<=500) then
			t_thetha(i)=-12*thetha(i)+7100
endif
!print*, i, "T(teta)=", t_thetha(i)
end do
...
!print*, i, p_teta(i) !p_teta si p_a nu sunt bune
...
!print*,  m(teta),  f_ip(teta), tg(teta), z(teta), k(teta), norm(teta), f_sigma(teta), i
enddo


!p(t,teta)=p(teta) 	v(teta)=v(teta)
!pa(vx)=p_a(i)		vx=i
!pe(vx)=p_e(i)		vx=i
!pleg(va)=p_la(i)	va=i
!plege(ve)=p_le(i)	ve=i

!do i=1,720
!print*, p_a(i), p_ev(i), p_la(i), p_le(i)
!enddo
